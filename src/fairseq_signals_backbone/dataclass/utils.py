...
from fairseq_signals_backbone.dataclass import Config
...
def gen_parser_from_dataclass(
    ...
    for k in dataclass_instance._get_all_attributes():
        ...
        if field_name is None:
            continue
        elif inspect.isclass(field_type) and issubclass(field_type, Dataclass):
...
def _override_attr(
    ...
    if not inspect.isclass(data_class) or not issubclass(data_class, Dataclass):
        return overrides
    ...
    for k, v in data_class.__dataclass_fields__.items():
        ...
        if (
            ...
            # if type is int but val is float, then we will vrash later - try to convert here
            ...
            try:
                ...
                pass # ignore erros here, they are often from interpolation args
...
def override_module_args(args: Namespace) -> Tuple[List[str], List[str]]:
    ...
    for k in Config.__dataclass_fields__.keys():
        overrides.extend(
            _override_attr(k, Config.__dataclass_fields__[k].type, args)
        )
    
    if args is not None:
        if hasattr(args, "task"):
            from fairseq_signals_backbone.tasks import TASK_DATACLASS_REGISTRY
        ...
        from fairseq_signals_backbone.utils.registry import REGISTRIES
        ...
        if hasattr(args, "arch"):
            from fairseq_signals_backbone.models import ARCH_MODEL_REGISTRY, ARCH_MODEL_NAME_REGISTRY
            ...
            if args.arch in ARCH_MODEL_REGISTRY:
                ...
                if dc is not None:
                    ...
                    # oevrride model params with those exist in args
...
def convert_namespace_to_omegaconf(args: Namespace) -> DictConfig:
    ...
    # configs will be in fairseq_signals/config after installtion
    ...
    with omegaconf_no_object_check():
        ...
        if cfg.lr_scheduler is None and getattr(args, "lr_scheduler", None):
            ...
            from fairseq_signals_backbone.optim.lr_scheduler import LR_SCHEDULER_REGISTRY
        ...
        if cfg.criterion is None and getattr(args, "criterion", None):
            ...
            from fairseq_signals_backbone.criterions import CRITERION_REGISTRY
...
def merge_with_parent(dc: Dataclass, cfg: Dataclass, remove_missing=False):
    if remove_missing:
